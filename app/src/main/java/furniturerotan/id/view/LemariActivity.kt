...
class LemariActivity : AppCompatActivity() {
    private var binding : ActivityLemariBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val call: Call<Barang?>? =
            apiInterface.getAllProductByKategori("Bearer $cookie","Lemari")
            ...
            override fun onResponse(call: Call<Barang?>, response: Response<Barang?>) {
                if (response.isSuccessful) {
                    ...
                    for (i in dataArrayList?.indices!!) {
                        recyclerViewAdapter = RecyclerViewAdapter(dataArrayList!!, this@LemariActivity)
